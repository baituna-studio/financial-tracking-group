  ...
  const handleSubmit = async (formData: FormData) => {
    ...
    try {
      ...
      const res = await fetch(`/api/categories/${category.id}`, {
        ...
        body: JSON.stringify({ name, description, icon, color, type, group_id: groupId }),
      })
      const data = await res.json()
      if (!res.ok || !data.ok) throw new Error(data.error || "Gagal memperbarui kategori")

      toast({ title: "Kategori diperbarui", description: "Perubahan telah disimpan." })
      onSuccess()
      onClose()
    } catch (e: any) {
      toast({ title: "Gagal memperbarui kategori", description: e?.message || "Error", variant: "destructive" })
    } finally {
      setIsLoading(false)
    }
  }
        ...
        <form action={handleSubmit}>
          <div className="grid gap-4 py-4">
            ...
            <div className="grid gap-2">
              ...
              <Select name="type" defaultValue={category.type || "Pengeluaran"} required>
                <SelectTrigger>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="grid gap-2">
                ...
                <Input id="icon" name="icon" defaultValue={category.icon || "folder"} />
              </div>
              <div className="grid gap-2">
